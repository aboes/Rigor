...
from tkinter.ttk import Label, Button, Entry, Radiobutton, Checkbutton, OptionMenu
...
class AlexRigor(tk.Tk):
    """Enthält die Logik um alle Frames im selben Fenster zu zeigen."""
...
class StartPage(tk.Frame):
    """Enthält die GUI-Elemente zur Darstellung der Startseite UND die Logik um die Themen-
    und Subthemen-Auswahl zu laden, sowie die Funktion um die Quiz-Abfrage zu starten."""
    def __init__(self, parent, controller):
        ...
        opmenu_theme = OptionMenu(opmenugroup, opt_theme, "Thema wählen", *THEMEN)
        ...
        opmenu_subset = OptionMenu(opmenugroup, opt_subset)
        ...
        for text, mode in MODES:
            radio_modes = Radiobutton(radiogroup, text=text, variable=self.opt_modes, value=mode)
            radio_modes.pack(side=tk.LEFT)
        radiogroup.pack(side=tk.TOP, padx=10, pady=5)

        buttongroup = tk.Frame(self)
        but_option = Button(buttongroup, text="Optionen",
                            command=lambda: controller.show_frame(OptionsPage))
        but_option.pack(side=tk.LEFT)

        but_start = Button(buttongroup, text="Start", state=tk.DISABLED,
...
class OptionsPage(tk.Frame):
    """Enthält die GUI-Elemente für die Options-Seite,
    sowie die Funktion um die gewählten Optionen zu speichern."""
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        ...
        label = Label(self, text="Optionen", font=LARGE_FONT)
        label.pack(padx=10, pady=20)
        ...
        check_capit = Checkbutton(self, text="Grossschreibung ignorieren", variable=self.cvar_capit, width=35)
        check_capit.pack()
        check_help1 = Checkbutton(self, text="1. Hilfe: Ersten Buchstaben anzeigen", variable=self.cvar_help1, width=35)
        check_help1.pack()
        check_help2 = Checkbutton(self, text="2. Hilfe: Anzahl Buchstaben anzeigen", variable=self.cvar_help2, width=35)
        check_help2.pack()
        check_alter = Checkbutton(self, text="Alternative Schreibungen zulassen", variable=self.cvar_alter, width=35)
        check_alter.pack()
        check_time = Checkbutton(self, text="Zeitbeschränkung (5 Sekunden)", variable=self.cvar_time, width=35, state=tk.DISABLED)
        check_time.pack()

        but_back = Button(self, text="zurück", command=self.save_and_back)
...
class QuizPage(tk.Frame):
    """Enthält die GUI-Elemente UND die Programm-Logik für die eigentliche Vokabel-Abfrage."""
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        ...
        label_bmrk = Label(self, textvariable=self.bemerkung, font=SMALL_FONT)
        label_bmrk.pack()

        self.useranswer = tk.StringVar()
        self.entry_answ = Entry(self, textvariable=self.useranswer, font=ANSWER_FONT, justify=tk.CENTER)
        self.entry_answ.pack(fill=tk.X)

        self.pindicate = tk.StringVar()
        self.lab_pindi = Label(self, textvariable=self.pindicate, foreground="red", font=SMALL_FONT)
        self.lab_pindi.pack()

        self.response = tk.StringVar()
        label_resp = Label(self, textvariable=self.response, font=LARGE_FONT)
        label_resp.pack()

        frame_button = tk.Frame(self)
        frame_button.pack(padx=10, pady=10)
        self.but_help = Button(frame_button, text="Hilfe", command=self.givehelp)
        self.but_help.pack(side=tk.LEFT)

        self.but_enter = Button(frame_button, text="Enter", command=self.checkanswer)
        self.but_enter.pack(side=tk.LEFT, ipadx=20)
        self.but_next = Button(frame_button, text="Weiter", command=self.nextquestion)
